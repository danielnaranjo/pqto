                ...
                */
                class Service extends CI_Controller {
                    ...
                    public function index($id=FALSE){
                        $data = $this->Service_model->listar($id);
                        echo json_encode($data);
                    }
                    public function view($id){
                        $data = $this->Service_model->obtener($id);
                        echo json_encode($data);
                    }
                    public function add(){
                        $data = $this->Service_model->registrar($this->input->post(NULL, TRUE));
                        if($data){
                            echo json_encode($data);
                        }
                    }
                    public function delete($id){
                        $data = $this->Service_model->deletear($id);
                        echo json_encode($data);
                    }
                    public function update(){
                        $id = $this->input->post("service_id");
                        $data = $this->input->post(NULL, TRUE);
                        $res = $this->Service_model->updatear($id, $data);
